...
class music_cog(commands.Cog):
    ...
    @ commands.command(
        name="venha",
        aliases=['entre', 'entrar', 'adentre', 'vemca', 'vem ca', 'vem cá', 'vemcá'],
    ...
    async def join(self, ctx):
        if ctx.author.voice:
            ...
            await ctx.send(f'Luna entrou {userChannel}')
        else:
            await ctx.send("Você precisa estar conectado em uma call bobaum.")

    @ commands.command(
        name='saia',
        aliases=['tchau', 'sair', 'vaza', 'some', 'rala'],
